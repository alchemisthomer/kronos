...
function loadFromFile(
  path: string,
  loginUrlOverride: string | undefined,
): SalesforceCredentials {
  let raw: string;
  try {
  ...
  if (missing.length) {
    throw new CredentialError(
      `Credentials file at "${path}" is missing required fields: ${missing.join(', ')}. ` +
        `Compare against ./credentials/salesforce.example.json.`,
      'credentials-incomplete',
    );
  }
...
/**
 * Redact credential substrings from a string so error messages emitted to
 * stderr or the run manifest never leak the password or session token.
 * Called from src/index.ts before any error output.
 */
export function redactCredentials(
  ...
  if (creds?.securityToken)
    out = out.split(creds.securityToken).join('[REDACTED_SECURITY_TOKEN]');
