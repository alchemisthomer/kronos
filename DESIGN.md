# Kronos — Design (v0.5, design-frozen after convergence-round review)

**Document state:** Design frozen — v0.5 (folds in the initial four-LLM review round AND a subsequent convergence pass by all four reviewers)
**Convergence declaration:** [`docs/CONVERGENCE-DECLARATION.md`](docs/CONVERGENCE-DECLARATION.md) — declares the architectural surface frozen; implementation begins with the first paid engagement against olympus-grid.
**MVP sequencing decision:** engine-first. Full order in the convergence declaration §"MVP sequencing decision."
...
**Changes from v0.4 (via convergence-round review, landed in v0.5):** Design frozen per Grok's convergence directive. MVP sequencing decision recorded (engine-first: schemas → CLI coordinator → authorization policy engine → typed executor → oracle engine → evidence manifest → cleanup verifier → L14 engagement → static report → scorecard → React runner → OAuth → continuous plane). New `docs/CONVERGENCE-DECLARATION.md` names the frozen surface and the implementation-sprint prerequisites (Priority 1: domain-model JSON schemas, finding-taxonomy refinement, plausibility-monitor unit-safety, autonomous-authorization hardening, executable revocation; Priority 2: OAuth server security, tool-binding contract deduplication, evidence-tier interaction with Salesforce backbone, consulting-operations prerequisites; Priority 3: OSCAL implementation, community-contribution flow, scorecard visualization, dynamic Salesforce capacity inference). New `docs/HOW-AN-ADOPTER-STARTS.md` one-page adoption flow. **GTM sanity-check corrected**: v2 addendum acknowledges the AI-app-builder assurance market is NOT greenfield (CheckVibe, Vibe App Scanner, VibeEval, Scanbee, Lovable native security, Replit Security Center, StackHawk Vibe, Aikido all ship in the space); repositioned kronos as "independent release-assurance layer for AI-built applications" rather than "another security scanner"; retired "naive builder" term; revised pricing ($750-$5,000 Launch Review, not $29/month scan); split into Kronos Preflight (passive lead-gen) vs Kronos Launch Review (paid productized service). **README stale claims fixed** (Claude convergence P1): removed "seven novel properties not present in any prior methodology," fixed link from renamed SEVEN-CLAIMS.md to INVENTIVE-CONCEPT-CANDIDATES.md, added competitor-landscape section. **INVENTIVE-CONCEPT-CANDIDATES body-preamble contradiction fixed** (Claude convergence P1): body language softened to match preamble; each candidate qualified with acknowledged prior art; specific-conjunction claim added per Grok's recommendation. **Scorecard executive-projection mandate** (Grok): single default executive view = traffic-light on five critical dimensions + pinned/latest headline; multi-axis detail behind drill-down. **Founding-incident day-of-week error corrected** (ChatGPT): 2026-07-17 was Friday, not Sunday. **Deferred to implementation sprint per convergence declaration**: finding-taxonomy refinement (TargetFinding / AssuranceGap / FrameworkIssue / SafetyEvent / OperationalEvent), plausibility-monitor unit-safe formulas, autonomous-authorization hardening details, OAuth server security, tool-binding contract deduplication, executable domain-model schemas. These are legitimately implementation-time work, not design work.
...
The structural coupling of dual-use tooling with a first-class authorization artifact, in which the authorization is both a legal record and an executable configuration, is a claimed novel property of the framework (INVENTIVE-CONCEPT-CANDIDATES §7).
...
Kronos claims seven novel properties, described conceptually in [`methodology/INVENTIVE-CONCEPT-CANDIDATES.md`](methodology/INVENTIVE-CONCEPT-CANDIDATES.md). A detailed patent disclosure paralleling `foundation/eos/PATENT-DISCLOSURE-DRAFT.md` will be authored after this design converges through the cross-LLM review process.
...
The **conjunction** of all seven, applied to software assurance, is the inventive step claimed. Individual properties have partial analogues in prior work (referenced in `methodology/INVENTIVE-CONCEPT-CANDIDATES.md`); the conjunction is not present.
...
**(OPEN — deferred to operator + IP counsel, flagged in v0.1)** Is the strategy patent play, open-source moat play, or hybrid? — Deferred-decision block added to INVENTIVE-CONCEPT-CANDIDATES. Downstream framing depends on this decision.
...
- [`methodology/INVENTIVE-CONCEPT-CANDIDATES.md`](methodology/INVENTIVE-CONCEPT-CANDIDATES.md) — the novel-property claims (with a deferred-decision block on patent-vs-open-source strategy)
...
- `docs/alignment/<standard>.md` — per-industry-standard mapping documents; first-priority set: OWASP ASVS 5.0.0, NIST CSF 2.0, NIST AI RMF 1.0, SOC 2 TSC 2017, AWS Well-Architected
